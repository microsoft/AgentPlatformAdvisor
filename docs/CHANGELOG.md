...
- **Refreshed `.github/copilot-instructions.md`.** The changelog convention it documented had gone stale in a way that would actively undo work: it told future sessions that work in progress belongs under `## Unreleased`, a section that was deliberately removed, so anyone following the instructions would have recreated it. It now documents the real convention (dated by commit date, newest first, `(later)` for a second same-day batch) and says not to add `## Unreleased`. Added a "Share links are untrusted input" subsection to the share-link contract, since two security bugs have now come from treating URL params as trusted — per-question option-ID validation (hard rules fire on option ID alone, so an ID smuggled into the wrong question changes the winner), the `^\d{8}$` guard on `d=`, the rule that any path setting `fastTrack` must also force card mode, and a preference for DOM nodes over `innerHTML` wherever a URL value is involved. Also recorded that `og:description` must equal the on-page welcome copy, that `meta.last_updated` is hand-maintained and must be bumped when content changes, that `npm test` needs PyYAML before Playwright ever starts, and the two recurring test traps (loose regexes that pass on generic text, and probes outside `tests/e2e/` that Playwright silently ignores). Refreshed the spec list, which named 7 of the 16 files.

### Removed

- **Deleted `.github/improvements.md`.** It was a productization wishlist, and every item on it has since shipped: the version strip and footer "Last updated" indicator, the loading/welcome/error states, the stepped progress bar, markdown export, the "Did this help?" feedback loop, the prominently linked public changelog, and a README that leads with why the tool exists. Of the few items never built, three were deliberately declined rather than forgotten — a vanity domain would make a Microsoft-org tool read as unofficial third-party work, email-self needs a backend this app deliberately does not have, and a separate landing page would add a click before value when the welcome card already names the product, the problem, and every destination. The remaining item, whether to claim an `aka.ms` link, is a process question rather than code. Keeping a stale wishlist around implies work is outstanding when it is not. `TODO.md` P2.5 and P2.7 still record the items and their provenance.

### Changed

- **Fixed the social/SEO metadata, which was still serving the old build-framed pitch.** The welcome copy was corrected on the page, but `og:description` kept the previous sentence ("we'll recommend the best Microsoft agent platform for your scenario"). That is the text that unfurls in Teams and Slack and appears in search results, and since share links are a core feature here, more people meet this app through the unfurl than through the start page. `og:description` now carries the on-page sentence verbatim. Two further problems surfaced in the same block: `name="description"` was declared twice with identical content, so crawlers picked one non-deterministically, and `og:title` read "CAT Agent Platform Advisor" while both `<title>` and `og:site_name` read "Agent Platform Advisor" — a leftover from the shared Power CAT tooling whose logo and favicon were already stripped in an earlier release. Guarded by `tests/e2e/meta-tags.spec.js`, which pins the description meta to exactly one, asserts `og:description` equals the rendered welcome copy, requires the metadata to name both the "get work done" and "build" paths, and holds `og:title`/`og:site_name` to the real product name. All four proven non-vacuous against the old head.
