...
- **Copilot Studio 2026 capability refresh** — updated scoring and recommendation copy to reflect the latest Copilot Studio capabilities: new agent experience, generative orchestration, event-triggered workflows, computer use, MCP tools/resources, connected/child agents, A2A integrations, Microsoft IQ, Foundry IQ preview, per-user memory preview, model selection, built-in evaluation, monitoring, agent inventory, and Copilot Credits cost considerations. Copilot Studio now scores higher for background/event-triggered agents, Dataverse/custom connector/API integration, complex-but-low-code orchestration, and AI/ML personas while Foundry remains preferred for custom model training, arbitrary BYO model/runtime control, high-scale code-first orchestration, and custom retrieval architecture.
- **Data-source guidance split for Copilot Studio vs Foundry** — split the former direct/private data option into business-system integration (Dataverse, custom connectors, direct APIs: Copilot Studio strongest) and custom retrieval architecture (custom RAG, Azure AI Search, private indexes, Foundry IQ, engineering-managed retrieval: Foundry strongest). Updated `docs/SCORING.md` and `docs/FLOWCHART.md`; the scored matrix now has 1,920 answer combinations, with Copilot Studio winning 1,721 combinations.
- **Agent Builder 2026 capability refresh** — updated the scoring model and recommendation copy to reflect current Agent Builder capabilities: Microsoft 365 content, scoped web, uploaded files, Teams/Outlook/People knowledge, admin-enabled Microsoft 365 Copilot connectors, code interpreter, and image generation. Agent Builder is no longer described as SharePoint/OneDrive-only, and now scores higher for small-team no-code knowledge, connector-backed, web/uploaded-file, and lightweight content/data-analysis scenarios. It remains hard-zeroed for external audiences, custom app deployment, background/event-driven execution, direct business system integrations, custom retrieval architectures, and multi-step action workflows that submit forms or update external systems.
- **Audience and data-source scoring refined** — split the internal-audience question into small-team vs broad internal deployment, added a new content-analysis task option, added a web/uploaded-files data option, and clarified connector-backed business systems, direct business integrations, and custom retrieval architectures. Updated `docs/SCORING.md` and `docs/FLOWCHART.md`; Agent Builder now wins 60 combinations (up from 10) while Copilot Studio remains the default for broader deployment, actions, governance, and integrations.
- **Landing page delegate group + captions** — the welcome page now shows Cowork and Scout in a separate, centered group beneath the four build platforms, under an "Or delegate" divider label (mono, all-caps), keeping delegate agents visually distinct from build platforms. Each of the six tiles (four platforms + Cowork + Scout) now carries a short caption beneath its name describing what it is. Delegate cards match the platform tile width and stay centered with no empty grid slot.
- **Delegate path — Copilot Cowork & Microsoft Scout** — added a new prescreen path, *"I'd like a ready-made agent to do work for me,"* that routes to Cowork or Scout. These are personal agents you delegate work to (and extend with skills/plugins), not build platforms, so they are **not** part of the scored 5-question wizard and are not in `meta.platforms`. A two-question micro-decision (cadence: on-demand vs continuous; reach: Microsoft 365 vs desktop/browser/local) resolves the recommendation: **Scout** if the work is continuous *or* reaches beyond Microsoft 365; **Cowork** only if on-demand *and* Microsoft 365-scoped; otherwise both are shown as a complementary pair. New `cowork` and `scout` recommendation blocks in `apa.yaml`; result is shareable via a `dt=cowork|scout|both` URL param. Scout carries a Frontier-preview caveat.
- **Cowork promoted from M365 capability to its own destination** — the "Featured Capability" spotlight for Cowork was removed from the M365 Copilot recommendation card, and the Cowork mention was removed from the built-in M365 Copilot prescreen tile (now led by Researcher, Analyst, Facilitator, Interpreter). Cowork is now reached via the delegate path.
- **Copilot Cowork spotlight** — Cowork is now featured as a dedicated "Featured Capability" callout on the M365 Copilot recommendation card, appearing directly below the summary with its own eyebrow label, bold name (linking to the Cowork docs), tagline, and description. It has been removed from the first-party agents accordion list, where it was previously item #1 among 10 peers. This gives Cowork appropriate prominence as an autonomous task execution layer rather than treating it as just another first-party agent.
